...
let attack cmd (st:state) =
  ...
  | Attack c -> (match c with
      |(l, r, Left, i) ->
        let st' = {st with occupied_countries = update_countries st.occupied_countries l i []} in
        {st' with active_players = update_player st.player_turn st'.player_turn st.active_players []}
      |(l, r, Right, i) ->
        let st1 = {st with occupied_countries = update_countries st.occupied_countries r i []} in
        let st' = {st1 with active_players = update_player st.player_turn st1.player_turn st.active_players []} in
      ...
      |(l, r, Both, i) ->
        (let st' = {st with occupied_countries = update_countries (update_countries st.occupied_countries l i []) r i []} in
         {st' with active_players = update_player st.player_turn st'.player_turn st.active_players []}))
...
let give_card st1 st2 = if num_countries st1.player_turn st1.occupied_countries 0 < num_countries st1.player_turn st2.occupied_countries 0 then
    let st = {st2 with player_turn = {st1.player_turn with
                             cards =
                               ...
                               else Square::(st1.player_turn.cards)}} in
